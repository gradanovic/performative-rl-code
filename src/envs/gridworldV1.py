...
class GridworldV1():
    ...
    def __init__(self, beta, eps, gamma, sampling=False, n_sample=500, seed=1, max_sample_steps=100):
        ...
        self.agents = {}
        self.action_space = {}
        # agent 1 is the main agent (controls actor movement based on correct gird)
        self.action_space[1] = self.moves
        self.agents[1] = Agent(1, self.action_space[1])
        # agent 2 is the secondary agent (makes interventions based on wrong grid)
        self.action_space[2] = self.interventions
        self.agents[2] = Agent(2, self.action_space[2])
        # pertrub grid for agent 2 -- the pertrubed grid stays the same for all experiments
        self.eps = eps
        pertrubation_seed = 100
        pertrubation_rng  = np.random.default_rng(pertrubation_seed)
        self.grid_pertrubed = self.pertrub_grid(pertrubation_rng)
        # parameters
        self.gamma = gamma
        self.beta = beta
        # sampling
    ...
    def get_rewards(self, state_id, next_state_id):
        ...
        # agent 2
        rewards[2] = self.grid_pertrubed[state[0], state[1]]
    ...
    def _get_mQ(self, U, agent, fixed_agent):
        ...
        # initialize all state-move values to zero
        Q = np.zeros(shape=(self.dim, len(self.action_space[agent.id])), dtype='float64')
    ...
    def initialize_policies(self):
        ...
        V_star = self.value_iteration(agent)
        Q_star = self._get_Q(V_star, agent)
        ...
        V_br = self.best_response_value_iteration(agent, fixed_agent)
        Q_br = self._get_mQ(V_br, agent, fixed_agent)
        agent.policy = SoftMaxQ_Policy(agent.actions, Q_br, self.beta)
